...
BASE = ("/private/tmp/claude-501/-Users-otsov--superset-worktrees-8144834a-c76f-41f6-"
        ...
        "scratchpad/v8data")
HALO = 16           # lines around evidence for scoped number/label keeping
QUAL_PARA = 30     # extra full-context lines around evidence for qualitative
    ...
    for ev in evidence:
        ...
        for k in range(ev - 1, max(-1, ev - 40), -1):
            ...
            if found_title and found_period:
                break
        # wave-2 halo: near lines kept FULLY (answerer needs context), far halo scoped
        for k in range(max(0, ev - 4), min(n, ev + 5)):
            if lines_words[k]:
                keep_lines.add(k)
...
def main():
    ...
    out_path = os.path.join(BASE, "masks_w2.jsonl")
    file_cache = {}
    n_out = 0
    fracs = []
