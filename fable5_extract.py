#!/usr/bin/env python3
"""E28: Fable-5-as-extractive-compressor on the 17 never-answered questions.
...
OUT = os.path.join(SCRATCH, "fable5_extract")
...
EXTRACT_SYSTEM = """You are an expert financial-document evidence extractor. Given a question and a full SEC filing (plain text), produce a PURELY EXTRACTIVE compressed context that lets a small reader model answer the question.

Hard rules:
- Copy lines VERBATIM from the document: whole lines, character-for-character, in document order. No paraphrase, no commentary, no computed values, no headers you invent, no markdown.
- Include everything needed to answer: the statement/section title lines (e.g. "CONSOLIDATED STATEMENTS OF OPERATIONS"), the period/column header lines, any units line (e.g. "(in millions, except per share data)"), the evidence rows themselves, and neighboring rows needed to disambiguate labels.
- If the question requires arithmetic (ratios, growth, margins, averages), include EVERY component row for EVERY period involved, tightly juxtaposed.
- If the answer could plausibly come from more than one statement or period, include all candidates.
- Separate non-contiguous regions with a line containing exactly: ...
- Budget: aim for under 2500 tokens; smaller is better, but never drop something the reader would need.

Output ONLY the extracted lines (with ... separators). Nothing else."""
